...
Logging:
    Uses the shared project logger with output written to ``logs/rag_service.log``.
...
logger = get_logger(__name__, "rag_service.log")

TOP_K_RESULTS = int(settings.retrieval.get("top_k", 4))
SIMILARITY_THRESHOLD = float(settings.retrieval.get("similarity_threshold", 0.7))
